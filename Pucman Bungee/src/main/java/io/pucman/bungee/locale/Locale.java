...
public class Locale<P extends Plugin> extends BaseFile
{
    ...
    {
        if (this.getInstance().getResourceAsStream(this.getName()) != null) {
            TryUtil.sneaky(() -> Files.copy(this.getInstance().getResourceAsStream(this.getName()), this.getFile().toPath(), StandardCopyOption.ATOMIC_MOVE));
        }
